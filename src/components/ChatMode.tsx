...
export const ChatMode: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Send user message and context to AI service
      const response = await aiService.generateChatResponse(input, contextDocs);
      ...
      <ScrollArea className="flex-1 p-4 mb-4 border rounded-md">
        {messages.length === 0 ? (
          <div className="h-full flex items-center justify-center text-gray-400">
            <p>Ask Ketu a question to get started</p>
          </div>
        ) : (
